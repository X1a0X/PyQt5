...
class Involution(nn.Module):
    ...
    def __init__(self, c1, c2, kernel_size, stride):
        ...
        self.conv1 = Conv(c1, c1 // reduction_ratio, 1)  # compress channels
        self.conv2 = Conv(c1 // reduction_ratio, kernel_size**2 * self.groups, 1, 1)  # create kernel
...
class Involution_Res(nn.Module):

    def __init__(self, c1, c2, kernel_size, stride):
        super(Involution_Res, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        self.c1 = c1
        reduction_ratio = 4
...
class CAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_query = x.view(m_batchsize, C, -1)  # [bs, c, h*w]
        proj_key = x.view(m_batchsize, C, -1).permute(0, 2, 1)  # [bs, c, h*w]-->[bs, h*w, c]
        energy = torch.bmm(proj_query, proj_key)  # [bs, c, h*w] * [bs, h*w, c] = [bs, c, c]
        # ------------------------------------------------------------------------------------ #
...
class CAM_CASCADE(nn.Module):
    """ Channel attention module"""

    def __init__(self, in_dim):
        super(CAM_CASCADE, self).__init__()
        self.chanel_in = in_dim
...
class C3TR(C3):
    ...
    def __init__(self, c1, c2, n=1, Involution=False, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, Involution, shortcut, g, e)
        c_ = int(c2 * e)
...
class CBAM(nn.Module):
    def __init__(self, c1, c2, group=True,rate=4,reduction=32):
...
class CoordCBAM(nn.Module):
     def __init__(self, inp, oup, reduction=32):
        super(CoordCBAM, self).__init__()
        self.CBAM=CBAM(inp)
        self.CoordAtt=CoordAtt(inp)
     def forward(self, x):
        out1=self.CBAM(x)
        out2=self.CoordAtt(x)
...
class ResBlock_CBAM(nn.Module):
    def __init__(self, in_places, places, stride=1, downsampling=False, expansion=4):
        ...
        self.cbam = CoordAtt(c1=places*self.expansion,c2=places*self.expansion,)
...
class MSAM(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.spatial_attention = Involution(c1, c2, 3, 1)
...
class MSAM_CAM_INVO_CASCADE(nn.Module):
    def __init__(self, c1, c2):
        super(MSAM_CAM_INVO_CASCADE, self).__init__()
        self.channel_attention = CAM_CASCADE(c1)
        self.spatial_attention = Involution(c1, c2, 3, 1)
...
class MSAM_CAM_INVO_PARA(nn.Module):
    def __init__(self, c1, c2):
        super(MSAM_CAM_INVO_PARA, self).__init__()
        self.channel_attention = CAM_Module(c1)
        self.spatial_attention = Involution_Res(c1, c2, 5, 1)
...
class Res_MSAM(nn.Module):
    def __init__(self, c1, c2):
        super(Res_MSAM, self).__init__()
        self.channel_attention = ChannelAttentionModule(c1)
        self.spatial_attention = SpatialAttentionModule()
        self.coord=CoordAtt(c1,c2)
...
class ResBlock_MSAM(nn.Module):
    def __init__(self, in_places, places, stride=1, downsampling=False, expansion=4):
        super(ResBlock_MSAM, self).__init__()
        self.expansion = expansion
        self.downsampling = downsampling
        self.places = places // expansion

        self.bottleneck = nn.Sequential(
            nn.Conv2d(in_channels=in_places, out_channels=self.places, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(self.places),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(in_channels=self.places, out_channels=self.places, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(self.places),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Conv2d(in_channels=self.places, out_channels=places, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(places),
        )
        self.msam = MSAM(c1=places, c2=places,)
